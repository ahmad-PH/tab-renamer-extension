        ...
        if (thresholdPassed || hasRetriesLeft) {
            ...
            setTimeout(() => {
                void (async () => {
                    ...
                    log.debug(`From inside the timed-out function, looking at tab: ${JSON.stringify(tab.title)} and comparing it against the desired title: ${tabInfo.signature.title}"`);
